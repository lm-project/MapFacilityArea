...
define(['../msg/msg', '../poi/poiform'],function(Msg,PoiForm) {
	...
	var Map, Taskitem ,Polygon,polygons,checkEditPolygon;
	function init(map,taskitem ,polygon,polygonList,CheckEditPolygon) {
		Map = map, Taskitem = taskitem, Polygon = polygon,
		polygons = polygonList,checkEditPolygon = CheckEditPolygon;
	...
	function endEditPolygon(id, btn){
		if ( polygonEdit && polygonEdit.getPath().toString()!='' ) {
			if(!checkEditPolygon(polygonEdit)){
				return ;
			};
			if(confirm("是否保存数据") ) {
	            if( !submittask( id ) ){ 
