...
define(['../msg/msg',"../common/pointPath"], function(Msg,createPoint) {
	...
	function taskBeginEdit(){
		amplify.subscribe("task.begin.edit",function( data ) {
            ...
            loadFacilityArchive( data );//渲染右侧poiform，如果融achieve表查到此任务项，将polygon渲染到地图
        });
	}
	...
	function loadFacilityArchive( datas ) {
		var info = JSON.stringify(datas);
		$.ajax({
			type : "GET",
			url : path + '/common/taskitem/' + info,
	...
	function addPoiProEve(){
		$("#poiEditForm .tabs").on("click","div",function(){
			$(this).addClass("proAcition ").siblings().removeClass("proAcition ");
			var text = $(this).text();
			if(text=='显示设施区域属性'){
				$("#qaCon").css("display","none");
				$("#properties").css("display","block");
			}else{
				$("#properties").css("display","none");
				$("#qaCon").css("display","block");
			}
		});
	}
	...
	function removeQaDom() {
		addPoiProEve();
		if(Role.isQa){
			$("#properties input[id!=meshbox],select").attr("disabled","disabled");
			$("#editFacilityPro").text("显示设施区域属性");
			$("#exp,#feedback").attr("readonly","readonly");
			$("#exp,#feedback").removeAttr("required");
		}else{
			$("#editQA").text("");
